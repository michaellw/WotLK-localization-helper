...
local coords;
	if ( SelectBorroso == 0 ) then
		...
		OptionsButton2:Hide();
		OcultarName2:Show();
		DesenfoBoton:SetText(_G["_SHOW_ALL"]);
	...
	elseif ( SelectBorroso == 1 ) then
		...
		OcultarName2:Hide();
		DesenfoBoton:SetText(_G["_HIDE_ALL"]);
	end
...
function CharacterSelect_OnEvent(self, event, ...)
	...
	elseif ( event == "UPDATE_SELECTED_CHARACTER" ) then
		...
		if ( index == 0 ) then
			...
			CharSelectCreateCharacterButton:SetText(_G["CREATE_CHARACTER"])
			CharacterSelectDeleteButton:SetText(_G["DELETE_CHARACTER"])
			...
			CharSelectCharacterName2:SetText("|n" .. class .."|n".._G["LEVEL"].." " .. level);
			end
		end
			...
			if ( not name ) then
				...
				if( ghost ) then
					_G["CharSelectCharacterButton"..index.."ButtonTextInfo"]:SetFormattedText(CHARACTER_SELECT_INFO_GHOST, level, class);
						...
						_G["CharSelectCharacterButton"..index.."ButtonTextInfo"]:SetText("|cffffffffLevel "..level.."|r "..color..class.."|r");
